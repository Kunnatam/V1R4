...
if [ -f "$SERVER_PIDFILE" ]; then
    if kill -0 "$(cat "$SERVER_PIDFILE")" 2>/dev/null; then
        echo "TTS server already running (PID $(cat "$SERVER_PIDFILE"))"
    else
...
if [[ "${1:-}" == "--avatar" ]]; then
    ...
    if [ -f "$AVATAR_PIDFILE" ] && kill -0 "$(cat "$AVATAR_PIDFILE")" 2>/dev/null; then
        echo "Avatar already running (PID $(cat "$AVATAR_PIDFILE"))"
    else
        cd "$PROJECT_PATH"
        ...
        if kill -0 "$AVATAR_PID" 2>/dev/null; then
            echo "Avatar started (PID $AVATAR_PID)"
        else
            echo "Warning: Avatar process exited — check logs:"
...
echo "Stop: $PROJECT_PATH/server/scripts/stop.sh"
